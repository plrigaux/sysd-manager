...
pub fn save_column_config(
    ...
    let data_list: Vec<UnitColumn> = data.iter().map(|(_, up)| UnitColumn::from(up)).collect();
...
async fn save_config(
    ...
    if let Err(e) = create_dir_all(&sysd_manager_config_dir).await {
        error!(
