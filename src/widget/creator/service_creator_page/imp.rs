...
impl ObjectImpl for ServiceCreatorPageImp {
    fn constructed(&self) {
        ...
        let model = standard_output::outputs();
        self.standard_output_entry.set_model(Some(&model));
        self.standard_error_entry.set_model(Some(&model));
        ...
        factory.connect_bind(move |_factory, item| {
            ...
            let data = item
                .item()
                .and_downcast::<standard_output::StandardOutput>()
                .unwrap();
        ...
        self.standard_output_entry.add_controller(event_focus); //Can't clone controller
        let event_focus = gtk::EventControllerFocus::new();
        let this = self.obj().clone();
        event_focus.connect_leave(move |event| {
            if let Some(entry) = event.widget().and_downcast_ref::<adw::EntryRow>() {
...
impl ServiceCreatorPageImp {
    ...
    fn validate_standard_output_and_error(&self, entry: &adw::EntryRow, attribute: &str) {
        ...
        let name_err = if text.is_empty() {
            ...
            let mut name_err = CreateUnitErr::Malformed;
            for a in output_file_descriptor() {
                if a == text
                    && !matches!(
                        text,
                        "file:path" | "append:path" | "truncate:path" | "fd:name"
                    )
                {
                    name_err = CreateUnitErr::NoErr;
                    break;
                }
            }

            if CreateUnitErr::Malformed == name_err {
                const FILE: &str = "file:";
                const APPEND: &str = "append:";
                const TRUNCATE: &str = "truncate:";
                if let Some(path) = text.strip_prefix(FILE) {
                    name_err = Self::validate_path(path);
                } else if let Some(path) = text.strip_prefix(APPEND) {
                    name_err = Self::validate_path(path);
                } else if let Some(path) = text.strip_prefix(TRUNCATE) {
                    name_err = Self::validate_path(path);
                } else if text.starts_with("fd:") {
                    name_err = CreateUnitErr::NoErr;
                }
            }
            name_err
